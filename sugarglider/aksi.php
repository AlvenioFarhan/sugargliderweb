...
if ($mod == 'login') {
    ...
    if ($row) {
        ...
        redirect_js("index.php?m=dashboard");
    } else {
        print_msg("Maaf, Login Gagal! <br> Username atau Password Salah");
    }
} elseif ($act == 'logout') {
...
elseif ($mod == 'diagnosa_tambah') {
    ...
    $solusi = $_POST['solusi'];
    if ($kode == '' || $nama == '' || $detail == '')
        print_msg("Kolom yang bertanda * tidak boleh kosong!");
...
elseif ($mod == 'gejala_tambah') {
    ...
    if ($kode == '' || $nama == '')
        print_msg("Kolom tidak boleh kosong!");
    ...
    $db->query("DELETE FROM tb_rule WHERE kode_gejala='$_GET[ID]'");
...
/** PENGETAHUAN TAMBAH */
    ...
    if ($id_rule == '' || ($tanya == '' && $diagnosa == '') || $child == '')
        print_msg("Pilih <strong>Rule</strong>, salah satu <strong>Tanya/Diagnosa</strong>, dan <strong>Pilihan</strong>!");
    elseif ($db->get_row("SELECT * FROM tb_rule WHERE id_rule='$id_rule'"))
        print_msg("Id rule sudah terdaftar!");
    elseif ($ada)
        print_msg("Kombinasi Rule dan Pilihan sudah ada!");
    else {
        $kode = $tanya ?: $diagnosa;
